...
public fun send_tip(tip_jar: &mut TipJar, payment: Coin<SUI>, ctx: &mut TxContext) {
    let tip_amount = coin::value(&payment); // Get the value of the tip
    
    assert!(tip_amount > 0, EInvalidTipAmount); // Ensure the tip amount is positive

    transfer::public_transfer(payment, tip_jar.owner); // Transfer the tip to the owner

    tip_jar.total_tips_received = tip_jar.total_tips_received + tip_amount; // Update total tips received
    tip_jar.tip_count = tip_jar.tip_count + 1; // Increment tip count
